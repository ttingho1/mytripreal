...
<script>
	...
	$(function(){
		...
		$( "#checkout_date" ).datepicker( "setDate", 2); // 체크아웃 선택 시 디폴트로 focus 되는 날짜/ ex ( "setDate", 5) 는 오늘로부터 4일 뒤
	...
	// 체크아웃 날짜 변경시 n박의 n값을 input태그 value값에 동적으로 할당
	function nightNum(){
		$('input[name=nightNum]').attr('value', Number($('.nightNum').html().split('박')[0]));
	}
	...
	function validate(){
		...
		if($("input[name='room_no']:checked").val() == null){
			alert('방을 선택해주세요.');
			return;
		}
		$("input[name='room_no']").val($("input[name='room_no']:checked").attr('id'));
		frm.submit();
	}
...
/*
	function search(){
		...
		newForm.attr("action","/roomSearch");

		// create element & set attribute (input)
		newForm.append($("input[name=checkin_date]").val());
		newForm.append($("input[name=checkout_date]").val());
...
<form class="jss99" id="frm" method="post" action="/reservation">
	<!-- 예약번호 히든값 -->
		...
		<div class="jss20 bookingSection jss15">
			...
			<div class="MuiGrid-root jss22 sectionContainer MuiGrid-container">
					<div class="jss100">
						...
						<%--<input type="hidden" class="nightNum" value="" name="nightNum">--%>
						<div class="jss102">
							<div class="MuiFormControl-root MuiTextField-root jss125 MuiFormControl-fullWidth">
						...
						<div class="jss101">
							<%--<button class="btn btn-success btn-sm" onclick="search();">검색</button>--%>
								<button class="btn btn-success btn-sm" onclick="javascript:form.method='get', form.action='/roomSearch';">검색</button>
						</div>
					</div>
			</div>
		</div>
		...
		<ul class="jss26 itemList">
			<c:forEach var="roomVo" items="${roomVo}">
				<li>
					...
					<div class="jss34">
						<div class="jss35">
							<input type="radio" id="${roomVo.room_no}" name="room_no" class="jss36 box_none" value="${roomVo.room_fee}" style="width: 25%;" onclick="selectRoom(this);" readonly>
							<label for="${roomVo.room_no}" class="jss37 selectItemLabel" style="width: 41%;">
							<svg class="MuiSvgIcon-root" focusable="false" viewBox="0 0 24 24" aria-hidden="true" role="presentation">
						...
						<div class="jss38 priceArea">
							<div class="jss42 cost">
								<input type="text" class="box_none" value="${roomVo.room_fee}" readonly>원
							</div>
						</div>
					</div>
				</li>
